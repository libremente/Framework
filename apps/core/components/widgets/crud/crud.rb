    ...
    class Crud < Spider::Widget
        ...
        def prepare_widgets
            if @action == :table && @widgets[:table]
                ...
                table_page = nil
                #se presente un @fixed e in sessione ho un fixed diverso ho fatto una ricerca diversa => page = 1
                #se in sessione ho un fixed e @fixed a nil potrebbe essere una ricerca nuova senza fixed (o un ritorno da una edit o new ?)
                if (@fixed && !transient_session[:table_params].blank? && transient_session[:table_params]['fixed'] != @fixed) || (@fixed.blank? && !transient_session[:table_params].blank? && !transient_session[:table_params]['fixed'].blank?)
                    transient_session[:table_params]['page'] = nil #metto a nil per non avere una pagina che non esiste con una diversa ricerca
                end
            ...
            if @fixed
                ...
                #salvo il parametro fixed in sessione per controllarlo poi
                transient_session[:table_params]['fixed_cond'] = @fixed if transient_session[:table_params]
            end
